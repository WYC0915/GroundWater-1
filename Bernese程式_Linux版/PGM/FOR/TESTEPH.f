...
CC                05-MAR-12 : RD: CORRECT SIZE OF DUMMY ARGUMENT LIST IN
CC                                SR CONST
C*
...
C
       ...
       CHARACTER*3  ALF3
       CHARACTER*80 FILNAM,NAMFIL,TSTNAM,NUMBER
...
C      Write a fingerprint to the screen.
       ...
       WRITE(lfnErr,*) ' JPL TEST-EPHEMERIS program. ' //
     .                 ' Last modified August 28, 2007.'

C      Print the ephemeris constants.

       CALL  Const (NAMS, VALS, SS, NVS, NAMFIL, NUMBER)

       WRITE (lfnErr,'(/3F14.2/200(/2(A8,D24.16)))') SS,
     .                                         (NAMS(I),VALS(I),I=1,NVS)

C     Skip the file header comments.
      OPEN(10,FILE=TSTNAM,STATUS='OLD')
...
C     of the present version of the ephemeris
   ...
   2  READ(10,'(15X,D10.1,3I3,F30.20)',END=9)
...
C  arguments and do the testing outside of the subroutine.
      ...
      CALL  Pleph ( ET, NTARG, NCTR, R , NAMFIL)
...
c     tolerances for different types of quantities.
      ...
      IF ( MOD(LINE,npt) .EQ. 0 )  WRITE(lfnErr,200)
...
C
      IF (DEL .GE. 1.D-13) WRITE(lfnErr,201)
     . LINE,ET,NTARG,NCTR,NCOORD,XI,R(NCOORD),DEL
...
C++++++++++++++++++++++++
C
      SUBROUTINE FSIZER3(NRECL,KSIZE,NRFILE,NAMFIL,NUMBER)
C
C++++++++++++++++++++++++
C
...
C++++++++++++++++++++++++++
C
      SUBROUTINE PLEPH ( ET, NTARG, NCENT, RRD, NAMFIL )
C
C++++++++++++++++++++++++++
C  NOTE : Over the years, different versions of PLEPH have had a fifth argument:
C  sometimes, an error return statement number; sometimes, a logical denoting
...
C          (SEE THE DISCUSSION IN THE SUBROUTINE STATE)
  ...
  11  IF(FIRST) CALL state(zips,list,pv,pnut,NAMFIL,NUMBER)
      FIRST=.FALSE.
...
C     CHECK FOR NUTATION CALL
      ...
      IF(NTARG.NE.14) GO TO 97
        IF(IPT(35).GT.0) THEN
          LIST(11)=2
          CALL STATE(ET2,LIST,PV,RRD,NAMFIL,NUMBER)
          RETURN
        ELSE
          do i=1,4
          rrd(i)=0.d0
...
C++++++++++++++++++++++++++++++++
C
      SUBROUTINE STATE(ET2,LIST,PV,PNUT,NAMFIL,NUMBER)
C
C++++++++++++++++++++++++++++++++
C
...
C        CALL FSIZER1(NRECL,KSIZE,NRFILE,NAMFIL,NUMBER)
C        CALL FSIZER2(NRECL,KSIZE,NRFILE,NAMFIL,NUMBER)
        CALL FSIZER3(NRECL,KSIZE,NRFILE,NAMFIL,NUMBER)

      IF(NRECL .EQ. 0) WRITE(lfnErr,*)
     .  '  ***** FSIZER IS NOT WORKING *****'
...
C       GET LIBRATIONS IF REQUESTED (AND IF ON FILE)
  ...
  98  WRITE(lfnErr,198)ET2(1)+ET2(2),SS(1),SS(2)
 198  format(' ***  Requested JED,',f12.2,
     * ' not within ephemeris limits,',2f12.2,'  ***')

      stop

   99 WRITE(lenErr,'(2F12.2,A80)')ET2,'ERROR RETURN IN STATE'
...
C+++++++++++++++++++++++++++++
C
      SUBROUTINE CONST(NAM,VAL,SSS,N,NAMFIL,NUMBER)
C
C+++++++++++++++++++++++++++++
C
...
C  CALL STATE TO INITIALIZE THE EPHEMERIS AND READ IN THE CONSTANTS
      ...
      IF(FIRST) CALL STATE(zips,list,xx,xx,NAMFIL,NUMBER)
      first=.false.
