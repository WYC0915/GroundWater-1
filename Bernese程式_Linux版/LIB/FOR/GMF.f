...
C     output data
C     -----------
C     gmfh: hydrostatic mapping function and derivative wrt z
C     gmfw: wet mapping function and derivative wrt z
C
C     Johannes Boehm, 2005 August 30
C
C     Adaptation to Bernese GPS Software:  U. Hugentobler (14-08-2006)
C     Recursions for Legendre polynomials: O. Montenbruck (14-08-2006)
C     Adaptions for pi:                    A. Gaede       (28-02-2007)
C
      ...
      REAL(r8b),DIMENSION(10,10) :: V,W
      REAL(r8b) :: dmjd,dlat,dlon,dhgt,zd,x,y,z
      REAL(r8b),DIMENSION(2)     :: gmfh,gmfw

      INTEGER(i4b)               :: i,n,m,nmax,mmax
      REAL(r8b)                  :: doy,phh
      REAL(r8b)                  :: ah,bh,ch,aw,bw,cw
      REAL(r8b)                  :: ahm,aha,awm,awa
      REAL(r8b)                  :: c10h,c11h,c0h
      REAL(r8b)                  :: a_ht,b_ht,c_ht
      REAL(r8b)                  :: sine,cose,beta,gamma,topcon
      REAL(r8b)                  :: hs_km,ht_corr,ht_corr_coef

      REAL(r8b),DIMENSION(55)    :: ah_mean,bh_mean,ah_amp,bh_amp
      REAL(r8b),DIMENSION(55)    :: aw_mean,bw_mean,aw_amp,bw_amp
...
C Legendre polynomials (Cunningham)
      V(1,1) = 1.0D0
      W(1,1) = 0.0D0;
      V(2,1) = z * V(1,1);
      W(2,1) = 0.0;
...
c derivative
      gmfh(2)= gmfh(1)**2/topcon*cose*(1-gamma**2/ah*(1-beta**2/bh))
...
c derivative
      gmfh(2)= gmfh(2) + (cose/sine**2 -
     1                    topcon*cose/(sine+gamma)**2*
     2                    (1-gamma**2/a_ht*(1-beta**2/b_ht))) * hs_km
...
c derivative
      gmfw(2)= gmfw(1)**2/topcon*cose*(1-gamma**2/aw*(1-beta**2/bw))
