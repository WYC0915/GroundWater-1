...
SUBROUTINE writcrux000(filename,stacrux,title)

! -------------------------------------------------------------------------
! Purpose:    This subroutine writes the STACRUX file version 0.00
!
! Author:     H. Bock
!
! Created:    18-May-1999
...
!             04-Oct-2010 SL: downgraded to Stacrux Version 0.00
!             15-Oct-2012 RD/SL: Bernese GPS->GNSS Software
!
...
! Modules
! -------
  USE m_bern,   ONLY: i4b,r8b,shortLineLength,lfnLoc,lfnErr,pgmVer
...
! -----------------
  IF (PRESENT(title)) THEN
    titLine = title
  ELSE
    titLine = 'STAINFO FILE OF BERNESE GNSS SOFTWARE VERSION ' // PGMVER
  ENDIF
...
! ------------------------------
  IF ( stacrux%technique == 'GNSS' .OR. stacrux%technique == '' ) THEN
    WRITE(lfnloc,'(A)') 'STATION NAME          FLG          FROM                   TO&
         &         RECEIVER TYPE         ANTENNA TYPE          REC #   ANT #&
    ...
    DO icrx = 1, stacrux%ninfo
      ...
      CALL timst2(2,2, tdummy(1:2), datstr)
      WRITE(lfnloc,                                                        &
           '(A16,4x,2X,A3,2X,A40,2X,2(A20,2X),2(I6,2X), 3(F8.4,2X),A22,2X,A)') &
...
! ----------------
  ELSEIF ( stacrux%technique == 'SLR ' ) THEN
    ...
    DO icrx = 1, stacrux%ninfo
      ...
      CALL timst2(2,2, tdummy(1:2), datstr)
      WRITE(lfnloc,                                                        &
           '(A16,4x,2X,A3,2X,A40,2X, (A8,2X), 3(F10.4,2X),A22,2X,A)')  &
           stacrux%stainfo(icrx)%stanam, stacrux%stainfo(icrx)%flg, datstr,&
