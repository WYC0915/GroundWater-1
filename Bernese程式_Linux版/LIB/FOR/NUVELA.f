      ...
      SUBROUTINE NUVELA(PSIT,T0,X0,Y0,Z0,T,X,Y,Z,VX,VY,VZ)
C
C  ORIGINALLY ABSMO_NUVEL (SEE BELOW) MODIF. BY ZUHEIR ALTAMIMI TO:
C   - COMPUTE VELOCITIES BY NNR-NUVEL1
C   - THEN TO SCALE THEM BY 0.9562 IN ORDER TO OBTAIN NNR-NUVEL1A VELOCITIES
C
...
C    &        /'PCFC',    -0.0907,     0.2902,    -0.5976,
C    &         'AFRC',     0.0532,    -0.1856,     0.2348,
C    &         'ANTA',    -0.0494,    -0.1018,     0.2218,
C    &         'ARAB',     0.4003,    -0.0311,     0.4049,
C    &         'AUST',     0.4695,     0.3072,     0.3762,
C    &         'CARB',    -0.0109,    -0.2027,     0.0945,
C    &         'COCO',    -0.6249,    -1.2944,     0.6544,
C    &         'EURA',    -0.0590,    -0.1434,     0.1887,
C    &         'NAZC',    -0.0921,    -0.5138,     0.5756,
C    &         'NOAM',     0.0152,    -0.2155,    -0.0094,
C    &         'SOAM',    -0.0624,    -0.0906,    -0.0523,
C    &         'JUFU',     0.2995,     0.4805,    -0.2936,
C    &         'INDI',     0.3995,     0.0026,     0.4066,
C    &         'PHIL',     0.5913,    -0.4412,    -0.5976/
C
C   AM0-2
C
...
C
       ...
       VZ =  (ORX*Y0 - ORY*X0)
C
C  COMPUTE VELOCITIES BY NNR-NUVEL1A
C
       VX =  (ORY*Z0 - ORZ*Y0)*0.9562D0
       VY =  (ORZ*X0 - ORX*Z0)*0.9562D0
       VZ =  (ORX*Y0 - ORY*X0)*0.9562D0
C
C   FINISH UP
C
      RETURN
