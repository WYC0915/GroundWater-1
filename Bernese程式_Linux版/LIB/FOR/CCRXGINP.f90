...
SUBROUTINE ccrxginp(lfnerl,lfnprl,maxfil,maxcom,ncom0,coment, &
                    runby,iflflg,nfleph,fileph,              &
                    char4,devout,tcoll1,tcoll2,filseq)

! -------------------------------------------------------------------------
! Purpose:    Concatenate RINEX navigation files
!
! Remarks:    Option reading routine for standalone program CCRINEXG
!             for new menu system
!
...
!             16-May-2003 RD: Use SR gttimwin instead of SR readsess
...
! ---------------
  ...
  CHARACTER(LEN=25)             :: srname ='sr ccrxginp (pg ccrinexg)'
...
! --------------------------
  CALL pritit('CCRINEXG','Concatenate GLONASS RINEX navigation files')
  CALL prflna
...
! -----------------
  ...
  DO icom=1,ncom
    ...
    CALL ckoptl(0,keyWord, keyValue, 'sr ccrxginp', 'comment line', &
                  irc, irCode, maxVal=1, maxLength=LEN(coment(1)),  &
                  empty=' ')
    IF (LEN_TRIM(keyValue(1)) /= 0) THEN
      ncom0=ncom0+1
      CALL dimtst(0,2,2,'CCRXGINP','MAXCOM','Comment lines', &
                                            ' ',ncom0,maxcom,irc)
...
! ---------------
  ...
  CALL dimtst(1,2,2,'CCRXGINP','MAXFIL','number of input RINEX files', &
              ' ',nfleph,maxfil,irc)
  DO ifil=1,nfleph
    fileph(ifil)=keyValue(ifil)
    IF (fileph(ifil) /= keyValue(ifil)) THEN
      WRITE(lfnerr,"(/,' *** SR CCRXGINP: Filename too long.', &
                  &  /,'                  Name: ',A,           &
