...
SUBROUTINE menu_rao(keyWord, menuauxInp, output)

! -------------------------------------------------------------------------
! Purpose:    Set the default list for receiver antenna offset estimation
...
! Local Parameters
! ----------------
  CHARACTER(LEN=8), PARAMETER   :: srName = 'menu_rao'
...
! ----------------------------------
  ...
  IF (tstkey('RAOSTR')) THEN
    ...
    CALL ckoptu(1,'RAOSTR',keyValue,srName,                         &
...
! ------------------------------------------
  ...
  IF (tstkey('RAOSTR') .AND. irCode == 0) THEN

    ! Receiver name
    CALL ckoptl(1,'RAOSTR',hlpStr(1,:),srName,                      &
                'Receiver name in rec.ant.offset setup',irc,irCode, &
                maxVal=numAnt,empty=' ',result2=antList(1:numAnt)%recnam)

    ! Antenna name
    CALL ckoptl(1,'RAOSTR',hlpStr(2,:),srName,                      &
                'Antenna name in rec.ant.offset setup',irc,irCode,  &
                maxVal=numAnt,empty=' ',result2=antList(1:numAnt)%antnam)

    ! Antenna number, from
    CALL ckopti(1,'RAOSTR',hlpStr(3,:),srName,                      &
                'Antenna num. in rec.ant.offset setup',irc,irCode,  &
                maxVal=numAnt,ge=0,le=999999,empty=0,               &
                result2=antList(1:numAnt)%antnum(1))

    ! Antenna number, to
    CALL ckopti(1,'RAOSTR',hlpStr(4,:),srName,                      &
                'Antenna num. in rec.ant.offset setup',irc,irCode,  &
                maxVal=numAnt,ge=0,le=999999,empty=0,               &
    ...
    CALL ckoptc(1,'RAOSTR',hlpStr(6,:),(/ 'GPS    ','GLO    ',           &
    ...
    ! Apriori sigma, horizontal
    CALL ckoptr(1,'RAOSTR',hlpStr(7,:),srName,                      &
                'Sigma(h) in rec.ant.offset setup',irc,irCode,      &
                maxVal=numAnt,ge=0d0,empty=0d0,                     &
                result2=antList(1:numAnt)%sigmaH)

    ! Apriori sigma, vertical
    CALL ckoptr(1,'RAOSTR',hlpStr(8,:),srName,                      &
                'Sigma(v) in rec.ant.offset setup',irc,irCode,      &
                maxVal=numAnt,ge=0d0,empty=0d0,                     &
                result2=antList(1:numAnt)%sigmaV)
  ...
  IF (irCode /= 0) THEN
    ...
    ' ### SR MENU_RAO: The values in the (old) input setup are not valid.',  &
...
! ---------------------------------------------------
  ...
  CALL ckoptu(1,'RAOSETUP',newList%value,srName,                   &
...
! ------------------------
    IF ( .NOT. inList ) THEN
      ...
      antList(numAnt)%sigmaH    = 0d0
      antList(numAnt)%sigmaV    = 0d0
...
! ----------------------------------------------
  ...
  ALLOCATE(hlpStr(8,1),stat=iac)
  CALL alcerr(iac,'hlpStr',(/8,1/),srName)
  ...
  DO iAnt = 1, numAnt
    ...
    IF (antList(iAnt)%sigmaH > 0d0) THEN
      WRITE(hlpStr(7,1), '(A,F8.3,A)') qt, antList(iAnt)%sigmaH, qt // ' '
    ELSE
      hlpStr(7,1) = qt // qt // ' '
    ENDIF

    IF (antList(iAnt)%sigmaV > 0d0) THEN
      WRITE(hlpStr(8,1), '(A,F8.3,A)') qt, antList(iAnt)%sigmaV, qt // ' '
    ELSE
      hlpStr(8,1) = qt // qt // ' '
    ENDIF

    jAnt = jAnt + 1
    WRITE(output%value(jAnt),'(8(A,1X))') (TRIM(hlpStr(ii,1)),ii=1,8)
