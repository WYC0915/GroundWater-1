...
SUBROUTINE ckoptt(iStop,keyWord, keyString, srName, descr, irc0, ircSum, &
                  empty, ge, le, gt, lt, colTit, iLine, linTit, maxVal,  &
                  init, error, nError, nValid, result1, nResult, result2)

! -------------------------------------------------------------------------
! Purpose:    Checks the input option as a time value
!
...
!                  (if present).
!               4. The string is decoded with 3 integers. These three integer
!                  values are transformed into hour numbers (real).
!                  If the reading of the integer numbers fails a
!                  warning/error is set (see 3).
!               5. The result (hour) is taken and the range is checked using
!                  the optional "ge", "gt", "le", "lt" values (if present).
!                  If the result is outside the range a warning/error is
!                  set (see 3).
!
! Author:     R. Dach
!
...
!                             compiler problems
!             01-Apr-2003 RD: Negative values have to be possible
...
! optional output:
  ...
  REAL(r8b),        OPTIONAL :: result1    ! only one result value;
            ! To set simple real variables (hour corresponding to keyString).
  ...
  REAL(r8b),                  &
      DIMENSION(:), OPTIONAL :: result2    ! array of results from all entries
                                           ! (hour corresponding to keyString)
...
! -----------------------------------------
  IF (LEN_TRIM(descr) > 45) &
    ...
    ' ### SR CKOPTT: Dear programmer, please give a shorter description',  &
...
! -----------------------------
  IF (iStop /= 0 .AND. iStop /= 1) THEN
    ...
    ' *** SR CKOPTT: Dear programmer, you did a FATAL ERROR!!!', &
...
! -----------------------------------
  ...
  CALL alcerr(irc, 'result', (/nItem/), 'ckoptt')
...
! -------------------------
  ...
  CALL alcerr(irc,'irCode',(/nItem/),'ckoptt')
  irCode =  0
  maxLen = -1
...
! ----------------------------
    IF (LEN_TRIM(keyString(iItem)) > 0 .AND. irCode(iItem) == 0) THEN
      ...
      READ(keyString(iItem),*,ioStat=ios) (zeit(ii),ii=1,3)
      IF (ios /= 0 .OR. zeit(2) < 0 .OR. zeit(3) < 0) THEN
        irCode(iItem) = 2
      ELSE
        result(iItem) = abs(zeit(1)) + zeit(2) / 60d0 + zeit(3) / 3600d0
        hlpKey=ADJUSTL(keyString(iItem))
        IF (hlpKey(1:1) == '-') &
          result(iItem) = -result(iItem)
      ENDIF
...
! -----------------------
    IF (iErr == 1 .OR. iErr == 2) WRITE(lfnerr, '(/,A)') errStr(iStop) // &
       'A time string is expected for keyword "' // TRIM(keyWord) // '"'

    IF (iErr == 3) WRITE(lfnerr, '(/,A)') errStr(iStop) // &
       'The time for keyword "' // TRIM(keyWord) // '" exceeds the range.'
...
! --------------------------
    DO iItem = 1, nItem
      ...
      IF (iErr == 3) THEN
        zeit(1) = INT(   DABS(result(iItem)))
        zeit(2) = INT(  (DABS(result(iItem)) - DBLE(zeit(1))) * 60d0   )
        zeit(3) = NINT( (DABS(result(iItem)) - DBLE(zeit(1))  &
                                       - DBLE(zeit(2))/60d0)  * 3600d0 )
        IF (result(iItem) /= DABS(result(iItem))) zeit(1) = -zeit(1)

        hlpStr(2) = ' '

        IF (result(iItem) /= DABS(result(iItem)) .AND. zeit(1) == 0) THEN
          WRITE(hlpStr(2),'(A,I2.2,2(1X,I2.2))') '-',(zeit(ii),ii=1,3)
        ELSE IF (zeit(1) < 100 .AND. zeit(1) >= 0) THEN
          WRITE(hlpStr(2),'(3(1X,I2.2))') (zeit(ii),ii=1,3)
        ELSE
          WRITE(hlpStr(2),*) zeit(1)
          i1 = LEN_TRIM(hlpStr(2)) + 1
          i2 = i1 + 6
          WRITE(hlpStr(2)(i1:i2),'(2(1X,I2.2))') (zeit(ii), ii=2,3)
        ENDIF

        hlpStr(3) = 'Bad value:   " ' // TRIM(ADJUSTL(hlpStr(2))) // ' "'
...
! ------------------------------
    ...
    IF (iErr == 2) THEN
      WRITE(lfnerr,'(16X,A,A)') '             ', &
                         'The string could not be converted to a time!'
      WRITE(lfnerr,'(16X,A,A)') '             ', &
...
! ----------------------
    IF (iErr == 3) THEN
      ...
      DO ii = 1,2
        ...
        zeit(1) = INT(   DABS(rHlp(ii)) )
        zeit(2) = INT(  (DABS(rHlp(ii)) - DBLE(zeit(1))) * 60d0   )
        zeit(3) = NINT( (DABS(rHlp(ii)) - DBLE(zeit(1))  &
                                  - DBLE(zeit(2))/60d0)  * 3600d0 )
        IF (rHlp(ii) /= DABS(rHlp(ii))) zeit(1) = -zeit(1)

        hlpStr(ii) = ' '

        IF (rHlp(ii) /= DABS(rHlp(ii)) .AND. zeit(1) == 0) THEN
          WRITE(hlpStr(2),'(A,I2.2,2(1X,I2.2))') '-',(zeit(jj),jj=1,3)
        ELSE IF (zeit(1) < 100 .AND. zeit(1) >= 0) THEN
          WRITE(hlpStr(ii),'(3(1X,I2.2))') (zeit(jj),jj=1,3)
        ELSE
          WRITE(hlpStr(ii),*) zeit(1)
          i1 = LEN_TRIM(hlpStr(ii)) + 1
          i2 = i1 + 6
          WRITE(hlpStr(ii)(i1:i2),'(2(1X,I2.2))') (zeit(jj), jj=2,3)
        ENDIF
