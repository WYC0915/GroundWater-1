...
SUBROUTINE ckoptd(iStop,keyWord, keyString, srName, descr, irc0, ircSum, &
                  empty, ge, le, gt, lt, colTit, iLine, linTit, maxVal,  &
                  init, error, nError, nValid, result1, nResult, result2)

! -------------------------------------------------------------------------
! Purpose:    Checks the input option as a date value
!
...
!                  (if present).
!               4. The string is decoded with 3 integers. The 1st one has to be
!                  a 4-digit year. These three integer values are transformed
...
! -----------------------------------------
  IF (LEN_TRIM(descr) > 45) &
    ...
    ' ### SR CKOPTD: Dear programmer, please give a shorter description',  &
...
! -----------------------------
  IF (iStop /= 0 .AND. iStop /= 1) THEN
    ...
    ' *** SR CKOPTD: Dear programmer, you did a FATAL ERROR!!!', &
...
! -----------------------------------
  ...
  CALL alcerr(irc, 'result', (/nItem/), 'ckoptd')
...
! -------------------------
  ...
  CALL alcerr(irc,'irCode',(/nItem/),'ckoptd')
  irCode =  0
  maxLen = -1
    ...
    DO iItem = 1, nItem
      ...
      IF (iErr == 3) THEN
        ...
        IF (zeit(1) < 10000d0) THEN
          WRITE(hlpStr(2),'(1X,I4,2(1X,I2.2))') (zeit(ii),ii=1,3)
        ELSE
...
! ----------------------
    IF (iErr == 3) THEN
      ...
      DO ii = 1,2
        ...
        hlpStr(ii) = ' '
        IF (zeit(1) < 10000d0) THEN
          WRITE(hlpStr(ii),'(1X,I4,2(1X,I2.2))') (zeit(jj),jj=1,3)
        ELSE
